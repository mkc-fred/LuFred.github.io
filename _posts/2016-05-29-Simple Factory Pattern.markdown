...
&emsp;&emsp;通过简单工厂模式，此时的财务不需要知道每个职位的薪资计算公式，她只需要通过工厂类的__*CreateEmployee*__ 方法通过传入相应的职位名，就能得到特定职位的对象，再调用__*GetSalaryResult*__ 方法就能计算出响应的薪资。  

如果部门新加了鼓励师职位怎么办？  
&emsp;&emsp;针对这个问题，我们只要创建一个鼓励师类，让其集成__*Employee*__ 类，重写计算薪资的__*GetSalaryResult*__ 方法。然后在工厂类中新加一个鼓励师的case即可。
